...
save_path = os.path.join(test_dir, f"saved_models/{game}_DDPG_{date_time}")
Path(save_path).mkdir(parents=True, exist_ok=True)
load_path = os.path.join(test_dir, f"saved_models/Drone_DDPG_20250410234442") # Need to update
...
class DDPGAgent():
    def __init__(self):
        ...
        if load_model == True:
            ...
            checkpoint = torch.load(load_path+'/Drone_DDPG.ckpt', map_location=device)
